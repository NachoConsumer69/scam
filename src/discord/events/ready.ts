...
export const execute = async (client: Client) => {
  client.user?.setActivity("wickbot.com | Shard192", {
    type: ActivityType.Watching,
  });

  const channelToSend = client.channels.cache.get(
    config.verify.channelId
  ) as TextChannel;
  (await channelToSend.messages.fetch({ limit: 50 })).forEach((message) =>
    message.author.bot ? message.delete() : null
  );
