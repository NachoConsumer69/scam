...
export const initialiseEmbed = (client: Client) => {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `${allEmojis.space}${allEmojis.success} **To access \`${guildName}\`, you need to pass the verification first.**\n${allEmojis.space}${allEmojis.space}${allEmojis.right} Press on the **Verify** button below.`
    );
...
export const successDMEmbed = (client: Client) => {
  ...
  const embed = new EmbedBuilder()
    .setColor("#f3b822")
    .setDescription(
      `${allEmojis.loading} Starting verification... Check your dms!`
    );
...
export const failedToDMEmbed = (client: Client) => {
  ...
  const embed = new EmbedBuilder()
    .setColor("#fc2323")
    .setDescription(
      `${allEmojis.cancel} **I wasn't able to DM you.. Open your DMs and try to reverify.**`
    );
...
export const failedVerificationEmbedReason = (
  client: Client,
  reason: string
) => {
  const allEmojis: IEmojis = emojis(client);
...
export const successVerificationEmbed = (client: Client) => {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `${allEmojis.success} You passed the verification successfully. You can now access \`${guildName}\`!`
    );
